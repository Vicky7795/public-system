...
const CitizenRegister = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!err.response) {
                ...
                setError(err.response?.data?.message || t('common.auth.reg_error_failed'));
            }
        } finally {
            setLoading(false);
        }
    ...
    const handleGoogleSuccess = async (response) => {
        ...
        try {
            ...
            setError(error.response?.data?.message || t('common.auth.google_reg_failed'));
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="relative z-10">
                    ...
                    <div className="space-y-6 mt-20">
                        <h2 className="text-4xl xl:text-5xl font-extrabold text-[#111827] leading-[1.1] tracking-tight">
                            {t('common.auth.branding.register_hero_text')}
                        </h2>
                        <p className="text-lg font-medium text-gray-500 max-w-md leading-relaxed">
                            {t('common.auth.branding.register_hero_sub')}
                        </p>
                    </div>
                    ...
                    <div className="mt-16 space-y-8">
                        <div className="flex items-center gap-4 group">
                            ...
                            <div>
                                <h4 className="font-bold text-gray-900 uppercase text-xs tracking-widest">{t('common.auth.branding.secure_profile_title')}</h4>
                                <p className="text-sm text-gray-400 font-semibold tracking-tight">{t('common.auth.branding.secure_profile_desc')}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-4 group">
                            ...
                            <div>
                                <h4 className="font-bold text-gray-900 uppercase text-xs tracking-widest">{t('common.auth.branding.direct_engagement_title')}</h4>
                                <p className="text-sm text-gray-400 font-semibold tracking-tight">{t('common.auth.branding.direct_engagement_desc')}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-4 group">
                            ...
                            <div>
                                <h4 className="font-bold text-gray-900 uppercase text-xs tracking-widest">{t('common.auth.branding.fast_track_title')}</h4>
                                <p className="text-sm text-gray-400 font-semibold tracking-tight">{t('common.auth.branding.fast_track_desc')}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="w-full max-w-xl animate-in fade-in slide-in-from-bottom-4 duration-700">
                    ...
                    <div className="bg-white/85 backdrop-blur-xl rounded-[24px] border border-white/40 shadow-2xl p-6 sm:p-12 relative overflow-hidden">
                        ...
                        <div className="mb-10 text-center lg:text-left">
                            <h3 className="text-[28px] font-bold text-[#111827] mb-2 tracking-[-0.4px] leading-[1.2] font-['Plus_Jakarta_Sans',sans-serif]">{t('common.auth.register_subtitle')}</h3>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="block text-[12px] font-semibold text-[#374151] uppercase tracking-[0.5px] px-1">{t('common.auth.full_name_label')}</label>
                                    <div className="relative group">
                                        ...
                                        <input
                                            ...
                                            placeholder={t('common.auth.full_name_placeholder')}
                                            value={formData.name}
                                ...
                                <div className="space-y-2">
                                    <label className="block text-[12px] font-semibold text-[#374151] uppercase tracking-[0.5px] px-1">{t('common.auth.phone_label')}</label>
                                    <div className="relative group">
                                        <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[#9CA3AF] group-focus-within:text-[#2563EB] transition-colors">
                                            <Phone size={18} />
                                        </div>
                            ...
                            <div className="space-y-2">
                                <label className="block text-[12px] font-semibold text-[#374151] uppercase tracking-[0.5px] px-1">{t('common.auth.email_addr_label')}</label>
                                <div className="relative group">
                                    <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[#9CA3AF] group-focus-within:text-[#2563EB] transition-colors">
                                        <Mail size={18} />
                                    </div>
                                    <input
                                        type="email" required
                                        className="w-full pl-12 pr-4 py-4 rounded-xl border-gray-100 bg-gray-50/50 focus:bg-white focus:border-[#2563EB] focus:ring-4 focus:ring-blue-500/5 outline-none transition-all font-medium text-[14px] text-[#111827] placeholder:text-[#9CA3AF]"
                                        placeholder={t('common.auth.email_addr_placeholder')}
                            ...
                            <div className="space-y-2">
                                <label className="block text-[12px] font-semibold text-[#374151] uppercase tracking-[0.5px] px-1">{t('common.auth.password_secure_label')}</label>
                                <div className="relative group">
                                    <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[#9CA3AF] group-focus-within:text-[#2563EB] transition-colors">
                                        <Lock size={18} />
                                    </div>
                            ...
                            <div className="pt-2">
                                ...
                                >
                                    {loading ? <Loader2 className="animate-spin text-[14px]" /> : (
                                        <>
                                            <span>{t('common.auth.register_btn')}</span>
                                            <ArrowRight size={18} />
                                        </>
                                    )}
                                </button>
                    ...
                    <div className="mt-10 flex flex-col items-center gap-3 opacity-60">
                        ...
                        <p className="text-[10px] text-[#9CA3AF] font-medium uppercase tracking-widest text-center">{t('common.auth.data_encrypted_reg')}</p>
                    </div>
                </div>
            </div>
        </div>
