...
import { useNavigate, Link } from 'react-router-dom';
import api from '../utils/api';
import { User, Phone, Mail, Lock, Loader2, ShieldPlus, Building2, ShieldCheck } from 'lucide-react';
...
const OfficerRegister = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        // Validation
        if (formData.role === 'Officer' && !formData.departmentId) {
            setError('Please select a department from the list.');
            return;
        }

        const requiredKey = formData.role === 'Admin' ? 'admin123' : 'officer123';
        if (secretKey !== requiredKey) {
            setError(`Invalid Secret Access Key for ${formData.role} role.`);
            return;
        }
        ...
        setError('');
        try {
            // Ensure password is set if empty (legacy support)
            const finalData = { ...formData };
            if (!finalData.password) finalData.password = requiredKey;

            const { data } = await api.post('/auth/register', finalData);
            localStorage.setItem('token', data.token);
            localStorage.setItem('user', JSON.stringify(data.user));
            setSuccess(true);
            
            const redirectPath = formData.role === 'Admin' ? '/admin' : '/officer';
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen p-4 sm:p-6 bg-slate-100 font-sans">
            <div className="w-full max-w-md mb-4 flex justify-start">
                <BackButton fallbackPath="/officer/login" />
            </div>
            <div className="bg-white p-6 sm:p-10 rounded-[2.5rem] shadow-2xl shadow-slate-200/50 w-full max-w-md border border-slate-200">
                <div className="flex justify-center mb-6">
                    <div className="bg-white p-3 rounded-2xl shadow-sm border border-slate-100">
                        <img src="/logo.png" alt="Government Logo" className="h-10 w-10 object-contain" />
                    </div>
                </div>
                <h2 className="text-3xl font-black text-slate-900 mb-2 text-center tracking-tight">Official Registration</h2>
                <p className="text-slate-500 text-center mb-8 font-medium italic text-sm">Portal for Administrators & Grievance Officers</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 gap-4">
                        ...
                        {/* Role Selection */}
                        <div>
                            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2 px-1">Access Role</label>
                            <div className="flex gap-2 p-1 bg-slate-50 rounded-2xl border-2 border-transparent">
                                <button 
                                    type="button"
                                    onClick={() => setFormData({...formData, role: 'Officer'})}
                                    className={`flex-1 py-2.5 rounded-xl text-xs font-black transition-all ${formData.role === 'Officer' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-400'}`}
                                >
                                    OFFICER
                                </button>
                                <button 
                                    ...
                                    className={`flex-1 py-2.5 rounded-xl text-xs font-black transition-all ${formData.role === 'Admin' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-400'}`}
                                >
                                    ADMIN
                                </button>
                            </div>
                        ...
                        <div>
                            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2 px-1">Official Email</label>
                            <div className="relative">
                        ...
                        <div>
                            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2 px-1">Create Password</label>
                            <div className="relative">
                    ...
                    <button type="submit" disabled={loading} className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-slate-300 hover:bg-black hover:-translate-y-0.5 transition-all flex justify-center items-center mt-6">
                        {loading ? <Loader2 className="animate-spin text-white" /> : `Register as ${formData.role}`}
                    </button>
                </form>
